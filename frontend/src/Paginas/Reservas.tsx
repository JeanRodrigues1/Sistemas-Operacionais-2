...
export const Reservas = () => {
    ...
    const salvarImgs = async (id: number) => {
        try {
            for (const file of files) {
                const form = new FormData();
                form.append("file", file);

                const response = await fetch(`http://localhost:3000/img/upload/${id}`, {
                    method: 'POST',
                    body: form,
                });
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                await salvarImgs(reservaCriada.id);
  ...
  return (
      ...
      <>
          ...
          <div className="text-black flex flex-col mt-8 ">
              <h1 className="text-xl font-bold">Reserva de sala</h1>
              ...
              <button className="btnReserva mt-8" onClick={handleSubmit}>Reservar</button>
